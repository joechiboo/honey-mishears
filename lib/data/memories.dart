...
/// 取名那一則回憶。
///
/// 不放進 [memoryEntriesForRules]：它不是梗、沒有關鍵字，而且已經發生之後
/// 要顯示的是她現在的名字——資料來自 [WifeIdentity] 而不是設定檔。
/// 名字可以改，這一則寫的是「她有了名字」這件事，所以改名不會再多一則。
MemoryEntry namingMemoryEntry(WifeIdentity identity) => MemoryEntry(
      id: MemoryId.naming,
      title: '她有了名字',
      detail: identity.hasName ? '你叫她「${identity.name}」。' : '你幫她取過名字。',
      hint: '她其實還沒有名字——你可以問她，也可以自己給她一個。',
    );
